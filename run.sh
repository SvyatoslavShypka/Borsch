#!/bin/sh
#Indicates the script will be run using the Bourne shell (sh).
#Disables Docker scan suggestions
export DOCKER_SCAN_SUGGEST=false
#Sets the timeout for stopping the container to 10 seconds.
stop_timeout=10
need_build=false
need_start=false
#Store the first and second command-line arguments.
option1="$1"
option2="$2"
#Causes the script to exit immediately if any command fails (useful for error handling).
set -e;
#print  -------------------
       #STARTING CONTAINER
       #-------------------
